...
<body>
	...
	var  tableBuilder;
				$(function(){
					...
					tableBuilder.setSortName(["roleName"]);
					...
					$.ajax({
				          ...
				          success: function (result) {
				        	  if(result.rcode ==  "000000"){
				        		  ...
				        		  var authIds = result.authIds;
				        		  authIds.forEach(function(element, index){
				        			  $('#auth').treeview('checkNode',
				        					  [ element, { silent: true }
				        			  ]);
				        		  });
				        		  var priceAuthIds = result.priceAuthIds;
				        		  priceAuthIds.forEach(function(element, index){
				        			  $('#priceauth').treeview('checkNode',
				        					  [ element, { silent: true }
				        			  ]);
				        		  });
				        		  $("#modifiedbtu").show();
                		...
                		$(function(){
                			...
                			$("#modifiedbtu").click(function(){
            					...
            					if($("#rolename").val() == ""){
            						...
            						updateRole(auth,priceauth);
            					}
                			});
                			
                			function updateRole(auth,priceauth){
                				$.ajax({
							          type: "post",
