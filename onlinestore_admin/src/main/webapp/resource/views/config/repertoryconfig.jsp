...
<body>
	...
	$(function(){
		...
		var formatter = function(value, row, index){
			var repertory = {};
			repertory.id = row.id;
			repertory.repertoryName = row.repertoryName;
			repertory.showIndex = row.showIndex;
			return '<a class="btn btn-info" onclick="editRepertory('+JSON.stringify(repertory).replace(/"/g, '&quot;')+')"><i class="glyphicon glyphicon-pencil"></i></a><a style="margin-left:10px;" class="btn btn-danger" onclick="deleteRepertory('+value+')"><i class="glyphicon glyphicon-trash"></i></a>';
		};
		m.set('操作', formatter); 
	...
	function editRepertory(repertory){
		$("#updatebtu").show();
		$("#addbtu").hide();
		$("#repertoryId").val(repertory.id);
		$("#repertoryIndex").val(repertory.showIndex);
		$("#repertoryName").val(repertory.repertoryName);
		$("#modbtu").click();
	}
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">编辑仓库配置</h4>
				</div>
				<div class="modal-body">
					<form role="form">
						<div class="form-group">
							<label for="repertoryIndex">地区序号</label> <input type="text"
								class="form-control" placeholder="请输入仓库序号" id="repertoryIndex">
							<input type="hidden" id="repertoryId">
							<label for="repertoryName">地区名称</label> <input type="text"
								class="form-control" placeholder="请输入仓库名称" id="repertoryName">
						</div>
					</form>
				</div>
                	...
                	$(function(){
                		$("#addbtu").click(function(){
                			if($("#repertoryName").val()==""){
                				 layer.msg("请输入仓库名称", {time : 1500, icon : 5});
                			}else{
                				$.ajax({
							          type: "post",
